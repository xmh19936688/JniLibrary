...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
    ...
    private void notifyOnDrawListener() {
    	if(mOnDrawListener != null){
    		uiHandler.removeMessages(0);
    		uiHandler.post(new Runnable() {
    			@Override
    			public void run() {
    				mOnDrawListener.onCurrentDrawingListener(getCurrentFrameIndex(), getCurrentPosition());
    			}
    		});
    	}
	}
    ...
    public interface OnDrawListener{
    	void onCurrentDrawingListener(int currentFrameIndex, int currentPosition);
    }
